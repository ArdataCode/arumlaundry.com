...
' );
	...
	if( Gen::GetArrField( $settCp, array( 'elmntrWdgtGal' ), false ) )
	{
		...
		foreach( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," elementor-widget-gallery ")][@data-settings]' ) as $item )
		{
			...
			if( $ctx -> cfgElmntrFrontend === null )
				$ctx -> cfgElmntrFrontend = _Elmntr_GetFrontendCfg( HtmlNd::FirstOfChildren( $xpath -> query( './/script[@id="elementor-frontend-js-before"]' ) ) );
	...
	if( Gen::GetArrField( $settCp, array( 'elmntrWdgtImgCrsl' ), false ) )
	{
		...
		foreach( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," elementor-widget-image-carousel ")][@data-settings]|.//*[contains(concat(" ",normalize-space(@class)," ")," elementor-widget-n-carousel ")][@data-settings]' ) as $item )
		{
			if( !ContentProcess_IsItemInFragments( $ctxProcess, $item ) )
				continue;
			...
			$itemStyleCont .= '
				' . $itemCssSel . ' .swiper:not(.swiper-initialized) > .swiper-wrapper
				{
					gap: 0;
				}
			';
	...
	top: var(--divi-top-lzl-height) !important;
...

		{
			...
			if( apply_filters( 'seraph_accel_jscss_addtype', false ) )
				$itemScript -> setAttribute( 'type', 'text/javascript' );
			$itemScript -> setAttribute( 'seraph-accel-crit', '1' );
			HtmlNd::SetValFromContent( $itemScript, "function seraph_accel_cp_diviHdr_calcSizes(a){const b=a.querySelector(\"#main-header\"),c=a.querySelector(\"#top-header\");a.documentElement.style.setProperty(\"--divi-hdr-lzl-height\",b.clientHeight-b.clientHeight+(c?c.clientHeight:0)+b.clientHeight+\"px\");a.documentElement.style.setProperty(\"--divi-top-lzl-height\",(c?c.clientHeight:0)+\"px\")}\n(function(a){function b(c){seraph_accel_cp_diviHdr_calcSizes(a)}a.addEventListener(\"seraph_accel_calcSizes\",b,{capture:!0,passive:!0});seraph_accel_izrbpb.add(function(){a.removeEventListener(\"seraph_accel_calcSizes\",b,{capture:!0,passive:!0})})})(document)" );
			$ctxProcess[ 'ndBody' ] -> insertBefore( $itemScript, $ctxProcess[ 'ndBody' ] -> firstChild );
		}
	}
}
						...
						for( $i = 0; $i < count( $aItemStyle ); $i++ )
						{
...

							{
								...
								switch( $alignX = Gen::GetArrField( $attrXy, array( 'x', $i ), '' ) )
								{
									...
									case 'r':
										$translate[ 0 ] = '-100%';
										$prefix = ( ( $baseAlign === 'slide' || $itemParent !== $itemFirstSlideTmp ) ? '' : '-1px * var(--lzl-rs-extra-x) + ' ) . '100% - ';
										break;

									default:
										$prefix = ( ( $baseAlign === 'slide' || $itemParent !== $itemFirstSlideTmp ) ? '' : '1px * var(--lzl-rs-extra-x) + ' );
	...
	if( $ctxProcess[ 'mode' ] == 'full' && $adjusted )
	{
...

		{
			...
			if( apply_filters( 'seraph_accel_jscss_addtype', false ) )
				$itemScript -> setAttribute( 'type', 'text/javascript' );
			$itemScript -> setAttribute( 'seraph-accel-crit', '1' );
			HtmlNd::SetValFromContent( $itemScript, str_replace( '_ADJUSTED_BUBBLES_', $adjustedBubbles ? '1' : '0', "function seraph_accel_cp_sldRev_calcSizes(a){for(var b=JSON.parse(a.getAttribute(\"data-lzl-widths\")),c=JSON.parse(a.getAttribute(\"data-lzl-widths-g\")),d=0;d<b.length&&!(window.innerWidth<b[d]);d++);d==b.length&&(d=b.length-1);var e=a.clientWidth/c[d];1<e&&(e=1);b=b[d]-a.clientWidth;0>b&&(b=0);c=(a.clientWidth-c[d])/2;0>c&&(c=0);a.style.setProperty(\"--lzl-rs-scale\",e);a.style.setProperty(\"--lzl-rs-diff-y\",b);a.style.setProperty(\"--lzl-rs-extra-x\",c)}\nfunction seraph_accel_cp_sldRev_bubblemorph_calcSizes(a){var b=Math.max(a.clientWidth,a.clientHeight)/5;a.style.setProperty(\"--sz\",\"\"+b+\"px\");a.style.setProperty(\"--flt\",'url(\"#'+a.id+\"-f-blur\"+(30<=b?\"\":\"-sm\")+'\")')}\n(function(a){function b(c){a.querySelectorAll(\"rs-module:not(.revslider-initialised)[data-lzl-widths]\").forEach(seraph_accel_cp_sldRev_calcSizes);_ADJUSTED_BUBBLES_&&a.querySelectorAll(\"rs-module:not(.revslider-initialised) .rs-lzl-cont.js-lzl-ing [data-bubblemorph]\").forEach(seraph_accel_cp_sldRev_bubblemorph_calcSizes)}a.addEventListener(\"seraph_accel_calcSizes\",b,{capture:!0,passive:!0});seraph_accel_izrbpb.add(function(){a.removeEventListener(\"seraph_accel_calcSizes\",b,{capture:!0,passive:!0});\na.querySelectorAll(\"rs-slides img\").forEach(function(c){c.src=c.getAttribute(\"data-lzl-src\")})})})(document,_ADJUSTED_BUBBLES_)" ) );
			$ctxProcess[ 'ndBody' ] -> insertBefore( $itemScript, $ctxProcess[ 'ndBody' ] -> firstChild );
		}
	}
}
...
function _ProcessCont_Cp_elmntrHdr( $ctx, &$ctxProcess, $doc, $xpath )
{
	...
	$item = HtmlNd::FirstOfChildren( $xpath -> query( './/*[@data-elementor-type="header"]' ) );
	if( $item && ContentProcess_IsItemInFragments( $ctxProcess, $item ) )
	{
		if( $ctx -> cfgElmntrFrontend === null )
			$ctx -> cfgElmntrFrontend = _Elmntr_GetFrontendCfg( HtmlNd::FirstOfChildren( $xpath -> query( './/script[@id="elementor-frontend-js-before"]' ) ) );

		foreach( $xpath -> query( './*[contains(concat(" ",normalize-space(@class)," ")," elementor-top-section ")][contains(@data-settings,"sticky")]', $item ) as $itemSticky )
		{
			$adjusted = true;

			$aStickyOn = array();
			$dataSett = @json_decode( $itemSticky -> getAttribute( 'data-settings' ), true );
			foreach( Gen::GetArrField( $dataSett, array( 'sticky_on' ), array() ) as $stickyOnViewId )
			{
				if( $view = $ctx -> cfgElmntrFrontend[ 'views' ][ $stickyOnViewId ] )
					$aStickyOn[] = array( $view[ 'cxMin' ], $view[ 'cxMax' ] );
			}

			$itemSticky -> setAttribute( 'data-lzl-sticky-widths', @json_encode( $aStickyOn ) );
		}

		if( $adjusted )
		{
			HtmlNd::AddRemoveAttrClass( $item, 'js-lzl-ing' );

			{
				...
				if( apply_filters( 'seraph_accel_jscss_addtype', false ) )
					$itemScript -> setAttribute( 'type', 'text/javascript' );
				$itemScript -> setAttribute( 'seraph-accel-crit', '1' );
				HtmlNd::SetValFromContent( $itemScript, 'seraph_accel_cp_elmntrHdr_calcSizes(document.currentScript.parentNode);' );
				$item -> appendChild( $itemScript );
			}
		}
	}
	...
	if( $ctxProcess[ 'mode' ] == 'full' && $adjusted )
	{
		...
		$ctxProcess[ 'aCssCrit' ][ '@\\.ekit-sticky--effects@' ] = true;
...

		{
			...
			if( apply_filters( 'seraph_accel_jscss_addtype', false ) )
				$itemsCmnStyle -> setAttribute( 'type', 'text/css' );
			HtmlNd::SetValFromContent( $itemsCmnStyle, '[data-elementor-type="header"].js-lzl-ing .elementor-top-section.lzl-sticky {
	position: fixed;
	width: 100%;
	margin-top: 0px;
	margin-bottom: 0px;
...
{
	$raw = _Elmntr_GetFrontendCfgEx( $itemInitCmnScr );
